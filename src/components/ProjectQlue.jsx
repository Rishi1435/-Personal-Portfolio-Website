    ...
    <div className="relative w-full h-44 md:h-48 flex items-center justify-center bg-[#030303] rounded-xl overflow-hidden border border-[color-mix(in_srgb,var(--color-accent)_12%,transparent)] group-hover:border-[color-mix(in_srgb,var(--color-accent)_28%,transparent)] transition-all duration-300">
      ...
      <svg viewBox="0 0 420 180" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        ...
        {[40,80,120,160,200,240,280,320,360,400].map(x => (
          <line key={x} x1={x} y1="0" x2={x} y2="180" stroke="color-mix(in srgb, var(--color-accent) 2.5%, transparent)" strokeWidth="1"/>
        ))}
        {[45,90,135].map(y => (
          <line key={y} x1="0" y1={y} x2="420" y2={y} stroke="color-mix(in srgb, var(--color-accent) 2.5%, transparent)" strokeWidth="1"/>
        ))}
        ...
        <circle cx="168" cy="32" r="4" fill="color-mix(in srgb, var(--color-accent) 15%, transparent)" stroke="var(--color-accent)" strokeWidth="0.8"/>
        <text x="176" y="35" fill="var(--color-accent)" fontSize="6" fontFamily="monospace" fontWeight="bold">qlue</text>
        {/* Status dot */}
        <circle cx="253" cy="32" r="2.5" fill="var(--color-accent)">
          {a && <animate attributeName="opacity" values="1;0.3;1" dur="2s" repeatCount="indefinite"/>}
        </circle>

        {/* ── SESSION INFO BLOCK ─────────────────────────────── */}
        <rect x="163" y="46" width="94" height="28" rx="4" fill="#0F0F0F" stroke="color-mix(in srgb, var(--color-accent) 8%, transparent)" strokeWidth="0.8"/>
        <text x="210" y="57" fill="rgba(255,255,255,0.5)" fontSize="5" fontFamily="monospace" textAnchor="middle">RESUME-BASED INTERVIEW</text>
        <text x="210" y="68" fill="var(--color-accent)" fontSize="7" fontFamily="monospace" fontWeight="bold" textAnchor="middle">Q4 of 8</text>

        {/* ── AI AVATAR / MIC INDICATOR ──────────────────────── */}
        <circle cx="210" cy="94" r="14" fill="#0A0A0A" stroke="color-mix(in srgb, var(--color-accent) 20%, transparent)" strokeWidth="1">
          {a && <animate attributeName="stroke" values="color-mix(in srgb, var(--color-accent) 20%, transparent);color-mix(in srgb, var(--color-accent) 60%, transparent);color-mix(in srgb, var(--color-accent) 20%, transparent)" dur="2.5s" repeatCount="indefinite"/>}
        </circle>
        {/* Mic icon inside */}
        <rect x="206.5" y="86" width="7" height="12" rx="3.5" fill="var(--color-accent)"/>
        <path d="M203 95 A7 7 0 0 0 217 95" stroke="var(--color-accent)" strokeWidth="1.2" fill="none" strokeLinecap="round"/>
        <line x1="210" y1="102" x2="210" y2="106" stroke="var(--color-accent)" strokeWidth="1.2" strokeLinecap="round"/>
        <line x1="207" y1="106" x2="213" y2="106" stroke="var(--color-accent)" strokeWidth="1.2" strokeLinecap="round"/>

        {/* Pulse rings around mic */}
        <circle cx="210" cy="94" r="18" fill="none" stroke="color-mix(in srgb, var(--color-accent) 15%, transparent)" strokeWidth="1">
        ...
        <circle cx="210" cy="94" r="22" fill="none" stroke="color-mix(in srgb, var(--color-accent) 8%, transparent)" strokeWidth="0.8">
        ...
        <rect x="163" y="114" width="72" height="3" rx="1.5" fill="var(--color-accent)">
        ...
        <text x="257" y="124" fill="var(--color-accent)" fontSize="4.5" fontFamily="monospace" textAnchor="end">76%</text>
        ...
        {[
          ...
          <g key={tab.label}>
            <rect x={tab.x} y="128" width={i===0?24:18} height="9" rx="2"
              fill={i===0 ? 'color-mix(in srgb, var(--color-accent) 20%, transparent)' : '#0C0C0C'}
              stroke={i===0 ? 'var(--color-accent)' : 'rgba(255,255,255,0.04)'} strokeWidth="0.6"/>
            <text x={tab.x + (i===0?12:9)} y="135" fill={i===0?'var(--color-accent)':'rgba(255,255,255,0.25)'}
        ...
        {[0,4,8,12,16,20,24,28,32,36,40,44,48,52,56,60,64,68,72,76,80,84,88].map((offset, i) => {
          ...
          return (
            <rect key={offset} x={163 + offset} y={158 - h/2} width="2.5" height={h} rx="1" fill="color-mix(in srgb, var(--color-accent) 50%, transparent)">
        ...
        <text x="12" y="28" fill="color-mix(in srgb, var(--color-accent) 50%, transparent)" fontSize="5.5" fontFamily="monospace" fontWeight="bold">STACK</text>
        {[
          ...
          <g key={label}>
            <rect x="8" y={y-8} width="62" height="10" rx="3" fill="#0A0A0A" stroke="color-mix(in srgb, var(--color-accent) 12%, transparent)" strokeWidth="0.6"/>
            <text x="39" y={y} fill="color-mix(in srgb, var(--color-accent) 60%, transparent)" fontSize="5" fontFamily="monospace" textAnchor="middle">{label}</text>
          </g>
        ))}

        {/* ── RIGHT PANEL: Scoring dimensions ───────────────── */}
        <text x="278" y="28" fill="color-mix(in srgb, var(--color-accent) 50%, transparent)" fontSize="5.5" fontFamily="monospace" fontWeight="bold">REAL-TIME SCORE</text>
        {[
          ...
          <g key={dim}>
            ...
            <text x="405" y={y+7} fill="var(--color-accent)" fontSize="5" fontFamily="monospace" textAnchor="end">{pct}%</text>
          </g>
        ))}

        {/* ── FCM Notification badge (bottom right) ──────────── */}
        <rect x="278" y="132" width="130" height="38" rx="6" fill="#0A0A0A" stroke="color-mix(in srgb, var(--color-accent) 15%, transparent)" strokeWidth="0.8">
          {a && <animate attributeName="opacity" values="0.6;1;0.6" dur="3s" repeatCount="indefinite"/>}
        </rect>
        <circle cx="288" cy="143" r="4" fill="color-mix(in srgb, var(--color-accent) 20%, transparent)" stroke="var(--color-accent)" strokeWidth="0.8"/>
        <text x="288" y="145.5" fill="var(--color-accent)" fontSize="5" fontFamily="monospace" textAnchor="middle">✓</text>
        ...
        <text x="296" y="161" fill="var(--color-accent)" fontSize="4" fontFamily="monospace">Claude 3 Haiku • Detailed analysis</text>
